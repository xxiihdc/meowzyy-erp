...
import { importShopeeOrders } from "@/app/orders/actions";
...
export default async function Home({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const importError = typeof params.import_error === "string" ? params.import_error : null;
  const importSummary = ["import_created", "import_updated", "import_failed"].every((key) => typeof params[key] === "string")
    ? `Import xong: tạo ${params.import_created} dòng, cập nhật ${params.import_updated} dòng, lỗi ${params.import_failed} dòng.`
    : null;
    ...
    <main className="min-h-screen bg-[#f8f9ff] text-[#0b1c30]">
      ...
      <div className="lg:pl-[280px]">
        ...
        <div className="mx-auto max-w-[1440px] p-5 md:p-8">
          <header><h1 className="text-2xl font-semibold tracking-tight">Tổng quan</h1><p className="mt-1 text-sm text-[#45464d]">Tổng hợp doanh thu thực nhận và đơn hoàn tất trên các sàn.</p></header>
          {importError ? <p role="alert" className="mt-4 rounded-md border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">{importError}</p> : null}
          {importSummary ? <p role="status" className="mt-4 rounded-md border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-700">{importSummary}</p> : null}

          <section className="mt-6 grid gap-6 sm:grid-cols-2 xl:grid-cols-4">
            <Card className={metricCardClassName}><CardHeader className="p-[18px] pb-2"><div className="flex items-center justify-between"><CardDescription className="text-xs font-semibold uppercase tracking-[0.06em] text-[#45464d]">Doanh thu thực nhận</CardDescription><CircleDollarSign className="size-5 text-[#0051d5]" /></div><CardTitle className="pt-3 text-3xl font-bold tracking-tight">{currency.format(revenue.actualRevenue)}</CardTitle></CardHeader><CardContent className="p-[18px] pt-1"><p className="inline-flex rounded-sm bg-[#eff4ff] px-2 py-1 text-xs font-semibold text-[#0051d5]">Đơn hoàn tất</p></CardContent></Card>
          ...
          <section className="mt-6 grid gap-6 xl:grid-cols-3">
            ...
            <Card className={metricCardClassName}><CardHeader className="p-6"><div className="grid size-10 place-items-center rounded-md bg-[#eff4ff]"><FileSpreadsheet className="size-5 text-[#0051d5]" /></div><CardTitle className="mt-4 text-xl">Import Shopee</CardTitle><CardDescription className="mt-1 leading-6">Chọn file Excel export gốc có sheet orders.</CardDescription></CardHeader><CardContent className="p-6 pt-0"><form action={importShopeeOrders} className="space-y-3"><input required name="file" type="file" accept=".xlsx" className="block w-full text-sm" /><Button type="submit" variant="outline" className="w-full rounded-sm">Import Excel <ChevronRight className="size-4" /></Button></form></CardContent></Card>
          </section>

          <section className={`mt-6 ${metricCardClassName} rounded-lg bg-white`}><CardHeader className="border-b border-slate-200 p-6"><CardTitle className="text-xl">Đơn hàng mới cập nhật</CardTitle><CardDescription className="mt-1">Danh sách được cập nhật sau mỗi lần import.</CardDescription></CardHeader><CardContent className="p-0">{orders.length === 0 ? <div className="grid min-h-64 place-items-center px-6 text-center"><div><ReceiptText className="mx-auto size-8 text-slate-300" /><p className="mt-3 font-medium">Chưa có đơn hàng</p><p className="mt-1 max-w-xs text-sm leading-6 text-slate-500">Dữ liệu sẽ xuất hiện sau khi phần import Excel được hoàn tất.</p></div></div> : <div className="overflow-x-auto"><Table><TableHeader><TableRow className="bg-slate-50"><TableHead className="pl-6">Sàn</TableHead><TableHead>Mã đơn</TableHead><TableHead>Trạng thái</TableHead><TableHead>Hoàn tất</TableHead><TableHead className="pr-6 text-right">Doanh thu thực nhận</TableHead></TableRow></TableHeader><TableBody>{orders.map((order) => <TableRow className="h-16" key={order.id}><TableCell className="pl-6 font-medium">{marketplaceLabel[order.marketplace]}</TableCell><TableCell>{order.marketplace_order_id}</TableCell><TableCell><Badge variant="secondary">{order.raw_status}</Badge></TableCell><TableCell>{order.completed_at ? new Date(order.completed_at).toLocaleDateString("vi-VN") : "—"}</TableCell><TableCell className="pr-6 text-right font-medium">{order.actual_revenue === null ? "—" : currency.format(Number(order.actual_revenue))}</TableCell></TableRow>)}</TableBody></Table></div>}</CardContent></section>
        </div>
      </div>
    </main>
  );
