...
- Khi PM đã xác nhận một Task hoàn thành, commit được push lên `origin/main` phải chứa `Closes #<task-number>`, `Fixes #<task-number>` hoặc `Resolves #<task-number>`. Git `post-push` hook sẽ chỉ xử lý các keyword rõ ràng này: chuyển item của Task sang `Done` trong Project #2 rồi đóng issue; không suy diễn hoàn thành từ diff hoặc nội dung commit không có keyword.
